...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Import CSV
    if ($action === 'import_csv' && !empty($_FILES['csv_file']) && $_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
        $handle = fopen($_FILES['csv_file']['tmp_name'], 'r');
        $headers = fgetcsv($handle); // skip header
        $imported = 0; $skipped = 0;
        while (($row = fgetcsv($handle)) !== false) {
            if (count($row) < 6) { $skipped++; continue; }
            $text = trim($row[0]); $level = trim($row[1]); $catSlug = trim($row[2]);
            $performer = trim($row[3]); $target = trim($row[4]); $rawModes = trim($row[5]);
            $perfOrient = trim($row[6] ?? 'any'); $targetOrient = trim($row[7] ?? 'any');
            if (!in_array($level, ['green','orange','purple','red']) || !$text) { $skipped++; continue; }
            $catId = null; foreach ($categories as $c) { if ($c['slug'] === $catSlug) $catId = $c['id']; }
            $modeArr = array_map('trim', explode(',', $rawModes));
            $modeArr = array_filter($modeArr, fn($m) => in_array($m, ['couple','wmw','mwm','mwmw']));
            if (empty($modeArr)) { $skipped++; continue; }
            $stmt = $pdo->prepare("INSERT INTO cards (text, level, category_id, performer, target, modes, performer_orientation, target_orientation, status, author_id) VALUES (?,?,?,?,?,?,?,?,'published',?)");
            $stmt->execute([$text, $level, $catId, $performer, $target, json_encode(array_values($modeArr)), $perfOrient, $targetOrient, $_SESSION['admin_user_id']]);
            $imported++;
        }
        fclose($handle);
        $importMsg = "Импортировано: $imported, пропущено: $skipped";
    }
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        if ($text && !empty($modes)) {
            if ($action === 'create') {
                $stmt = $pdo->prepare("INSERT INTO cards (text, level, category_id, subcategory_id, performer, target, performer_orientation, target_orientation, modes, status, author_id, is_finale, is_favorite, duration, prompt_override, image_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                $stmt->execute([$text, $level, $categoryId, $subcategoryId, $performer, $target, $performerOrient, $targetOrient, $modesJson, $status, $_SESSION['admin_user_id'], $isFinale, $isFavorite, $duration, $promptOverride ?: null, $finalImageUrl]);
            } else {
                $sql = "UPDATE cards SET text=?, level=?, category_id=?, subcategory_id=?, performer=?, target=?, performer_orientation=?, target_orientation=?, modes=?, status=?, is_finale=?, is_favorite=?, duration=?, prompt_override=?, updated_at=datetime('now')";
                $params = [$text, $level, $categoryId, $subcategoryId, $performer, $target, $performerOrient, $targetOrient, $modesJson, $status, $isFinale, $isFavorite, $duration, $promptOverride ?: null];
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    fputcsv($out, ['text','level','category_slug','performer','target','modes','performer_orientation','target_orientation']);
    $allCards = $pdo->query("SELECT c.*, cat.slug as cat_slug FROM cards c LEFT JOIN categories cat ON cat.id=c.category_id ORDER BY c.id")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($allCards as $c) fputcsv($out, [$c['text'], $c['level'], $c['cat_slug']??'', $c['performer'], $c['target'], implode(',',json_decode($c['modes']??'[]',true)??[]), $c['performer_orientation'], $c['target_orientation']]);
    fclose($out); exit;
}
...
// Filter by mode from URL
$filterMode = $_GET['mode'] ?? '';
if ($filterMode && in_array($filterMode, ['couple','wmw','mwm','mwmw'])) {
    $cards = array_filter($cards, function($c) use ($filterMode) {
        $modes = json_decode($c['modes'], true) ?? [];
        return in_array($filterMode, $modes);
    });
}
...
if ($search) {
    $cards = array_filter($cards, function($c) use ($search) {
        ...
        return mb_strpos(mb_strtolower($c['text']), $q) !== false;
    });
}

// Level filter
$filterLevel = $_GET['level'] ?? '';
if ($filterLevel && in_array($filterLevel, ['green','orange','purple','red'])) {
    $cards = array_filter($cards, function($c) use ($filterLevel) {
...
.search-bar{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:16px;margin-bottom:24px}
.field-hint{font-size:11px;color:rgba(255,255,255,.25);margin-top:2px;line-height:1.3}
</style>
</head>
<body>
<div class="header-bar">
    ...
    <div>
        ...
        <a href="/admin/cards.php?mode=couple" class="<?=$filterMode==='couple'?'active':''?>">♂♀ Пара</a>
        <a href="/admin/cards.php?mode=wmw" class="<?=$filterMode==='wmw'?'active':''?>">♀♂♀ ЖМЖ</a>
        <a href="/admin/cards.php?mode=mwm" class="<?=$filterMode==='mwm'?'active':''?>">♂♀♂ МЖМ</a>
        <a href="/admin/cards.php?mode=mwmw" class="<?=$filterMode==='mwmw'?'active':''?>">♂♀♂♀ МЖМЖ</a>
...
<div class="search-bar">
    <?php if (!empty($importMsg)): ?><div style="padding:12px;border-radius:12px;background:rgba(76,175,80,.15);border:1px solid rgba(76,175,80,.3);color:#4CAF50;margin-bottom:12px;font-size:13px"><?=e($importMsg)?></div><?php endif; ?>
    <form method="get" action="/admin/cards.php" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        <input type="text" name="search" value="<?=e($search)?>" placeholder="🔍 Поиск по тексту задания..." style="flex:1;min-width:200px;padding:10px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);color:#fff;font-size:14px;outline:none;font-family:inherit">
        <?php if ($filterMode): ?>
        <input type="hidden" name="mode" value="<?=e($filterMode)?>">
        <?php endif; ?>
        <select name="level" style="padding:10px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);color:#fff;font-size:14px;outline:none;font-family:inherit;appearance:none">
            <option value="">Все уровни</option>
            <option value="green" <?=$filterLevel==='green'?'selected':''?>>🟢 Зелёный</option>
            <option value="orange" <?=$filterLevel==='orange'?'selected':''?>>🟠 Оранжевый</option>
            <option value="purple" <?=$filterLevel==='purple'?'selected':''?>>🟣 Фиолетовый</option>
            <option value="red" <?=$filterLevel==='red'?'selected':''?>>🔴 Красный</option>
        </select>
        <button type="submit" style="padding:10px 20px;border-radius:60px;border:none;background:linear-gradient(90deg,#F663B3,#00A7D1);color:#fff;font-weight:600;cursor:pointer;font-size:13px">Найти</button>
        <?php if ($search || $filterLevel || $filterMode): ?>
        <a href="/admin/cards.php" style="padding:10px 20px;border-radius:60px;border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.6);font-size:13px">✕ Сброс</a>
        <?php endif; ?>
    </form>
    <div style="display:flex;gap:8px;margin-top:12px;align-items:center">
        <a href="/admin/cards.php?export=csv" style="padding:8px 16px;border-radius:60px;border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.6);font-size:12px">⬇ Экспорт CSV</a>
        <form method="post" enctype="multipart/form-data" style="display:inline-flex;gap:8px;align-items:center">
            <input type="hidden" name="action" value="import_csv">
            <input type="file" name="csv_file" accept=".csv" required style="font-size:12px;color:rgba(255,255,255,.4)">
            <button type="submit" style="padding:8px 16px;border-radius:60px;border:1px solid rgba(0,167,209,.3);background:rgba(0,167,209,.1);color:#00A7D1;font-size:12px;cursor:pointer">⬆ Импорт CSV</button>
        </form>
        <span style="font-size:11px;color:rgba(255,255,255,.2)">Формат: текст,уровень,slug_категории,performer,target,modes,perf_orient,target_orient</span>
    </div>
</div>
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div>
                ...
                <select name="level">
                    ...
                    <option value="purple" <?=($editCard['level']??'')==='purple'?'selected':''?>>🟣 Фиолетовый (экстрим)</option>
        ...
        <div class="form-row">
            <div>
                ...
                <div class="field-hint">♂♀ — кто именно делает действие. Если «любой» — подходит всем</div>
            </div>
            <div>
                ...
                <div class="field-hint">← на кого направлено действие. Например, «сними лифчик» → цель = партнёрша</div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div>
                ...
                <div class="field-hint">Кто может выполнять: гетеро — только с противоположным полом, би — с любым</div>
            </div>
            <div>
                ...
                <div class="field-hint">Ориентация цели: гетеро — действие с разнополым, би — с любым</div>
            </div>
        </div>
        ...
        <input type="number" name="duration" value="<?=e($editCard['duration']??'')?>" placeholder="Оставьте пустым — без таймера" style="width:100%;padding:10px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);color:#fff;font-size:14px;outline:none;font-family:inherit;margin-bottom:8px" min="10" max="3600">

        <label>Изображение</label>
        <div style="font-size:11px;color:rgba(255,255,255,.3);margin-bottom:8px;line-height:1.8">
            🎨 <a href="/admin/generate_js.php?card_id=<?=$editCard['id']?>" target="_blank" style="color:#F663B3">JS-генератор SVG</a> — без цензуры, локально<br>
            🧠 <a href="#" onclick="openAI('kandinsky')" style="color:#BE5AFF">Kandinsky (Fusion Brain)</a> ·
            <a href="#" onclick="openAI('neiro')" style="color:#00A7D1">Ai Neiro (Telegram)</a> ·
            <a href="#" onclick="openAI('kling')" style="color:#4CAF50">Kling AI</a><br>
            🔗 При клике текст карточки вставится в промпт AI-сервиса
        </div>
...
<tr>
    ...
    <td><span class="badge badge-<?=$card['level']?>"><?=['green'=>'🟢','orange'=>'🟠','purple'=>'🟣','red'=>'🔴'][$card['level']]??''?></span></td>
    ...
    <td style="font-size:12px"><?=$card['image_url']?'<a href="'.e($card['image_url']).'" target="_blank" style="color:#4CAF50">✓ есть</a>':'—'?> <?=$card['is_favorite']?'<span style="color:#F663B3">❤</span>':''?></td>
...
function openAI(service) {
    ...
    const encoded = encodeURIComponent(text.substring(0,100));
    const urls = {
        kandinsky: 'https://fusionbrain.ai?prompt=' + encoded,
        neiro: 'https://t.me/ai_neiro_bot?start=prompt_' + encoded,
        kling: 'https://kling.ai/generate?prompt=' + encoded
    };
