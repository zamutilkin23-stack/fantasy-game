...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $mode = $_POST['mode'] ?? ($_GET['mode'] ?? 'couple');
    $players = [];
...
<head>
<meta charset="utf-8">
<title>Начать игру — эротические фанты</title>
        ...
        <form method="post" class="form" id="gameForm">
            ...
            <div class="form__block">
                ...
                <div class="mode-select">
                    <label class="mode-opt <?=$mode==='couple'?'active':''?>">
                        <input type="radio" name="mode" value="couple" <?=$mode==='couple'?'checked':''?> onchange="updatePlayers()">
                        <span class="mode-opt__label">Пара (М+Ж)</span>
                        <span class="mode-opt__desc">Для двоих</span>
                    </label>
                    <label class="mode-opt <?=$mode==='wmw'?'active':''?>">
                        <input type="radio" name="mode" value="wmw" <?=$mode==='wmw'?'checked':''?> onchange="updatePlayers()">
                        <span class="mode-opt__label">ЖМЖ</span>
                        <span class="mode-opt__desc">Две девушки + парень</span>
                    </label>
                    <label class="mode-opt <?=$mode==='mwm'?'active':''?>">
                        <input type="radio" name="mode" value="mwm" <?=$mode==='mwm'?'checked':''?> onchange="updatePlayers()">
                        <span class="mode-opt__label">МЖМ</span>
                        <span class="mode-opt__desc">Два парня + девушка</span>
                    </label>
                    <label class="mode-opt <?=$mode==='mwmw'?'active':''?>">
                        <input type="radio" name="mode" value="mwmw" <?=$mode==='mwmw'?'checked':''?> onchange="updatePlayers()">
...
<footer class="footer">
    <div class="container">
        <div class="footer__grid">
            <div class="footer__col"><a href="/" class="logo">фанты.online</a><p class="footer__copy">© 2026 — 18+</p></div>
        </div>
    </div>
</footer>
</div>
...
function updatePlayers() {
    ...
    document.querySelector(`.mode-opt input[value="${mode}"]`).closest('.mode-opt').classList.add('active');
}
