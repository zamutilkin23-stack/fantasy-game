    ...
    if (CARD_ID !== null) {
        fetch('/game.php?action=complete&code='+CODE+'&card_id='+CARD_ID+'&turn='+CURRENT_TURN, {method:'POST'})
            .then(r=>r.json()).then(d=>{
                if(d.ok) {
                    if(d.gameEnded) { document.getElementById('resultTitle').textContent='🔥 Карточка-финал!'; document.getElementById('resultText').textContent='Игра завершена.'; document.getElementById('result').style.display='flex'; }
                    else if(d.stageJustAdvanced) showParticles();
                    setTimeout(()=>location.reload(), d.stageJustAdvanced ? 1500 : d.gameEnded ? 5000 : 800);
                }
            });
    }
}
